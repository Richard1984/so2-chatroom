...
void messages_queue_remove() {
  ...
  if (max_index != -1) {
    send_message(messages[max_index]->message, messages[max_index]->client_uid);
    free(messages[max_index]);
...
void *handle_client(void *arg) {
  ...
  char name[32];             // Nickname temporaneo del client
